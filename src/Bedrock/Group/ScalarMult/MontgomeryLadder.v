...
Section Gallina.
  ...
  Local Notation add := (M.add(field:=field)(char_ge_3:=M.char_ge_3)(a:=a)(b_nonzero:=b_nonzero)).
  Local Notation opp := (M.opp(field:=field)(a:=a)(b_nonzero:=b_nonzero)).
  Local Definition scalarmult := (@ScalarMult.scalarmult_ref _ add M.zero opp).
  Add Ring Private_ring : (F.ring_theory m) (morphism (F.ring_morph m), constants [F.is_constant]).

  Lemma montladder_gallina_equiv_affine n (Hn : 0 <= n < 2 ^ Z.of_nat count)
    (P : M.point) x (Hx : x = X0 P) :
    montladder_gallina n x = X0 (scalarmult n P).
  Proof.
    unshelve erewrite montladder_gallina_equiv,
      (M.montladder_correct(field:=field)
        (a:=a)(b:=b)(a24:=a24)(a24_correct:=a24_correct)
        (F.inv_0 _) (Z.of_nat count) ltac:(lia)
        n Hn _ x ); try solve [subst a; ring | trivial].
  ...
  Section MontLadder.
    ...
    Instance spec_of_montladder : spec_of "montladder" :=
      ...
      { requires tr mem :=
          ...
          K < 2 ^ Z.of_nat scalarbits /\
          Z.of_nat scalarbits <= 8*Z.of_nat (length Kbytes);
        ensures tr' mem' :=
          tr' = tr /\ (
          forall P, U = MontgomeryCurve.M.X0 (Fzero:=F.zero) P ->
          let OUT := MontgomeryCurve.M.X0 (Fzero:=F.zero) (scalarmult field Hm' a b b_nonzero K P) in 
  ...
  Hint Extern 1 (spec_of "felem_cswap") => (simple refine (spec_of_cswap)) : typeclass_instances.
    ...
    Proof.
      ...
      { DestructHead.destruct_head and; Tactics.ssplit; trivial; intros.
        pose proof LittleEndianList.le_combine_bound Kbytes.
